...
public final class Sorts {
    ...
    public static final Map<String, String> TRAIN_SETS = SortParser.map(
            "id", "train_set_id",
            "code", "data->>'code'",
            "name", "data->>'name'",
            "buildNumber", "(data->>'buildNumber')::int",
            "technicalName", "data->>'technicalName'",
            "snapshotVersion", "version"
    );

    public static final Map<String, String> CARRIAGES = SortParser.map(
            "id", "(c->>'id')::int",
    ...
    public static final Map<String, String> SEATS = SortParser.map(
            "id", "(s->>'id')::int",
            "seatNumber", "s->>'seatNumber'",
            "x", "(s->>'x')::double precision",
            "y", "(s->>'y')::double precision",
            "rotation", "(s->>'rotation')::double precision"
    );
}
