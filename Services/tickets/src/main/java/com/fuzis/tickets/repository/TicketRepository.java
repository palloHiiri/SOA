...
public class TicketRepository {
    ...
    private static final String BASE_SELECT =
            ...
            "FROM tickets t JOIN venues v ON v.id=t.venue_id " + LATEST_PRICE;
    ...
    public long insert(Connection c, String name, long venueId, String carriageNumber, String seatNumber, Boolean refundable, TicketType type) throws SQLException {
        ...
        try (PreparedStatement ps = c.prepareStatement(sql)) {
            ...
            ps.setString(3, carriageNumber);
            ps.setString(4, seatNumber);
            if (refundable==null) ps.setObject(5, null);
            else ps.setBoolean(5, refundable);
            if (type==null) ps.setObject(6, null);
            else ps.setString(6, type.name());
    ...
    public TicketResponse findById(Connection c, long id) throws SQLException {
        try (PreparedStatement ps = c.prepareStatement(BASE_SELECT+" WHERE t.id=?")) {
            ps.setLong(1, id);
            try (ResultSet rs = ps.executeQuery()) {
    ...
    TicketType type
        ...
        try (PreparedStatement ps = c.prepareStatement(sql)) {
            ...
            ps.setLong(7, id);
            if (ps.executeUpdate()!=1) throw new SQLException("Ticket disappeared during update");
        }
    }

    public void delete(Connection c, long id) throws SQLException {
        try (PreparedStatement ps = c.prepareStatement("DELETE FROM tickets WHERE id=?")) {
            ps.setLong(1, id);
            if (ps.executeUpdate()!=1) throw new SQLException("Ticket disappeared during delete");
        }
    }

    public boolean exists(Connection c, long id) throws SQLException {
        try (PreparedStatement ps = c.prepareStatement("SELECT 1 FROM tickets WHERE id=?")) {
            ps.setLong(1, id);
            try (ResultSet rs = ps.executeQuery()) {
                return rs.next();
            }
    ...
    public long countByVenue(Connection c, long venueId) throws SQLException {
        try (PreparedStatement ps = c.prepareStatement("SELECT COUNT(*) FROM tickets WHERE venue_id=?")) {
    ...
    Integer discount
        ...
        try (Connection c = dataSource.getConnection()) {
            return count(c, id, name, creationDate, venueId, trainSetId, carriageNumber, seatNumber, refundable, type, basePrice, discount);
        }
    }
    ...
    Integer discount
        ...
        appendFilters(sql, p, id, name, creationDate, venueId, trainSetId, carriageNumber, seatNumber, refundable, type, basePrice, discount);
    ...
    List<SortPart> sorts
        ...
        List<Object> p = new ArrayList<>();
        appendFilters(sql, p, id, name, creationDate, venueId, trainSetId, carriageNumber, seatNumber, refundable, type, basePrice, discount);
        sql.append(" ORDER BY ").append(sorts.stream().map(s->s.expression()+" "+s.direction()).reduce((a, b)->a+", "+b).orElse("t.id ASC"));
    ...
    public long countTickets() throws SQLException {
        try (
            Connection c = dataSource.getConnection();
            PreparedStatement ps = c.prepareStatement("SELECT COUNT(*) FROM tickets");
            ResultSet rs = ps.executeQuery()
        ) {
            rs.next();
            return rs.getLong(1);
    ...
    String seatNumber, Boolean refundable, TicketType type, List<SortPart> sorts) throws SQLException {
        ...
        sql.append(" ORDER BY ").append(sorts.stream().map(s->s.expression()+" "+s.direction()).reduce((a, b)->a+", "+b).orElse("t.id ASC")).append(" LIMIT ? OFFSET ?");
        p.add(size);
        p.add(offset);
        try (
            Connection c = dataSource.getConnection();
    ...
    private static TicketResponse map(ResultSet rs) throws SQLException {
        ...
        r.setVenue(new VenueReference(rs.getLong("venue_id"), rs.getString("venue_name"), rs.getInt("train_set_id")));
