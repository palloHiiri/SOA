...
public class CdcRepository {
    ...
    @Inject
    public CdcRepository(DataSource dataSource) { this.dataSource = dataSource; }

    public void insertSnapshot(int trainSetId, String data, long version) throws SQLException {
        try (Connection c = dataSource.getConnection()) { insertSnapshot(c, trainSetId, data, version); }
    }
    ...
    public void insertSnapshot(Connection c, int trainSetId, String data, long version) throws SQLException {
        ...
        try (PreparedStatement ps = c.prepareStatement(sql)) {
            ps.setInt(1, trainSetId); ps.setString(2, data); ps.setLong(3, version); ps.executeUpdate();
        }
    }

    public Optional<CdcSnapshot> findLatest(int trainSetId) throws SQLException {
        try (Connection c = dataSource.getConnection()) { return findLatest(c, trainSetId); }
    }
