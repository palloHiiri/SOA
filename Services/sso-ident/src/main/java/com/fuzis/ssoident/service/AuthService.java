...
public class AuthService {
    ...
    private Mono<Challenge> getChallenge(UUID challengeId) {
        return redis.opsForValue().get(challengeKey(challengeId)).flatMap(value -> {
            try {
                return Mono.just(mapper.readValue(value, Challenge.class));
            }
            catch (Exception e) {
                return Mono.error(e);
            }
    ...
    private String challengeKey(UUID id) {
        return "sso:mfa:challenge:" + id;
    }
    private String attemptsKey(UUID id) {
        return "sso:mfa:attempts:" + id;
    }
    private String resetKey(UUID id) {
        return "sso:password-reset:code:" + id;
    }
    private String resetAttemptsKey(UUID id) {
        return "sso:password-reset:attempts:" + id;
    }
